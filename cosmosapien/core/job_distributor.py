...
class JobDistributor:
    ...
    def __init__(self, config_manager: ConfigManager, model_library: ModelLibrary, local_manager: LocalModelManager):
    ...
    def _execute_single_task(self, job_request: JobRequest) -> JobResult:
        ...
        model_key = f"{routing_decision.selected_provider}:{routing_decision.selected_model}"
    ...
    def _execute_parallel_task(self, job_request: JobRequest) -> JobResult:
        ...
        available_models = [model for model in job_request.models if self._is_model_available(model)]
        ...
        if results:
            ...
            best_result = max(results, key=lambda r: self.model_status.get(r.model_used, ModelStatus("", "", True)).success_rate)
            return best_result
    ...
    def _execute_load_balanced(self, job_request: JobRequest) -> JobResult:
        ...
        best_model = min(available_models, key=lambda m: (
    ...
    def create_job(self, prompt: str, job_type: JobType = JobType.SINGLE_TASK, 
                   models: Optional[List[str]] = None, **kwargs) -> JobRequest:
        """Create a new job request."""
        job_id = f"job_{int(time.time())}_{hashlib.md5(prompt.encode()).hexdigest()[:8]}"
        
        if models is None:
            # Use smart routing to select models
            routing_decision = self.smart_router.smart_route(prompt)
            if routing_decision.selected_provider != "none":
                models = [f"{routing_decision.selected_provider}:{routing_decision.selected_model}"]
            else:
                models = []
