...
class SmartRouter:
    ...
    def _initialize_provider_quotas(self) -> Dict[str, ProviderQuota]:
        ...
        cloud_providers = {
            ...
            "grok": {
                "grok-beta": {"limit": 0, "cost": 0.0}  # No free tier info
            },
            "huggingface": {
                "gpt2": {"limit": 100, "cost": 0.0}  # Free tier
            }
        }
        ...
        for provider, models in cloud_providers.items():
            for model, info in models.items():
                ...
                custom_limit = smart_config.free_tier_limits.get(provider, {}).get(model, info["limit"])
                custom_cost = smart_config.custom_costs.get(provider, {}).get(model, info["cost"])
                
                quotas[key] = ProviderQuota(
                    provider=provider,
                    model=model,
                    free_tier_limit=custom_limit,
                    used_calls=self.usage_data.get("providers", {}).get(key, {}).get("used_calls", 0),
                    reset_date=self.usage_data.get("providers", {}).get(key, {}).get("reset_date", date.today().isoformat()),
    ...
    def _select_model_for_complexity(self, complexity: TaskComplexity, available_providers: List[ProviderQuota]) -> Tuple[str, str, str]:
        """Select the best model for a given complexity level."""
        
        # Sort providers by cost efficiency (local first, then by cost)
        sorted_providers = sorted(available_providers, key=lambda p: (not p.is_local, p.cost_per_call))
        
        if complexity == TaskComplexity.TRIVIAL:
            # Prefer local models for trivial tasks
            for provider in sorted_providers:
                if provider.is_local:
                    return provider.provider, provider.model, "Local model for trivial task"
            
            # Fallback to cheapest cloud model
            for provider in sorted_providers:
                if provider.remaining_calls > 0:
                    return provider.provider, provider.model, f"Cheapest available model ({provider.cost_per_call}$/call)"
            ...
            for provider in sorted_providers:
                if provider.is_local or provider.cost_per_call == 0.0:
                    if provider.remaining_calls > 0:
                        return provider.provider, provider.model, f"Free/local model for simple task"
            ...
            for provider in sorted_providers:
                if provider.remaining_calls > 0:
                    if provider.is_local:
                        return provider.provider, provider.model, "Local model for medium task"
                    elif provider.cost_per_call == 0.0:
                        return provider.provider, provider.model, "Free tier model for medium task"
                    else:
                        return provider.provider, provider.model, f"Available model ({provider.cost_per_call}$/call)"
            ...
            for provider in sorted_providers:
                if provider.remaining_calls > 0:
                    if "gpt-4" in provider.model or "claude-3-sonnet" in provider.model:
                        return provider.provider, provider.model, "High-capability model for complex task"
                    elif provider.is_local and "mixtral" in provider.model:
                        return provider.provider, provider.model, "Local high-capability model"
                    elif provider.cost_per_call <= 0.01:  # Cheap but capable
                        return provider.provider, provider.model, f"Cost-effective capable model ({provider.cost_per_call}$/call)"
            
            # Fallback to any available
            for provider in sorted_providers:
                if provider.remaining_calls > 0:
                    return provider.provider, provider.model, f"Available model for complex task ({provider.cost_per_call}$/call)"
            ...
            for provider in sorted_providers:
                if provider.remaining_calls > 0:
                    if "gpt-4" in provider.model or "claude-3-opus" in provider.model:
                        return provider.provider, provider.model, "Most capable model for expert task"
                    elif "claude-3-sonnet" in provider.model:
                        return provider.provider, provider.model, "High-capability model for expert task"
                    elif provider.is_local and ("mixtral" in provider.model or "llama3.2" in provider.model):
                        return provider.provider, provider.model, "Local high-capability model"
            
            # Fallback to any available
            for provider in sorted_providers:
                if provider.remaining_calls > 0:
                    return provider.provider, provider.model, f"Available model for expert task ({provider.cost_per_call}$/call)"
    ...
    def smart_route(self, prompt: str, explain_only: bool = False) -> RoutingDecision:
        ...
        selected_provider, selected_model, reasoning = self._select_model_for_complexity(complexity, available_providers)
        ...
        for provider in available_providers[:5]:  # Top 5 alternatives
            if provider.provider != selected_provider or provider.model != selected_model:
        ...
        if provider:
            ...
            for key in list(self.usage_data.get("providers", {}).keys()):
                if key.startswith(f"{provider}_"):
                    ...
                    self.usage_data["providers"][key]["reset_date"] = date.today().isoformat()
                    if key in self.provider_quotas:
