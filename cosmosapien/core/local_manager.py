...
class LocalModelManager:
    ...
    def detect_lm_studio(self) -> bool:
        ...
        if self.system == "darwin":  # macOS
            ...
            return Path("C:/Users/AppData/Local/Programs/LM Studio/LM Studio.exe").exists()
        elif self.system == "linux":
            return Path("/usr/local/bin/lmstudio").exists() or Path("/opt/lmstudio/lmstudio").exists()
        return False
    ...
    async def start_ollama(self) -> bool:
        ...
        try:
            if self.system == "darwin":
                ...
                subprocess.run(["launchctl", "start", "io.ollama.ollama"], capture_output=True)
            elif self.system == "linux":
    ...
    def get_installation_help(self, runner: str) -> str:
        ...
        if not instructions:
            return f"No installation instructions available for {runner} on {self.system}"
